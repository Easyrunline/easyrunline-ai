type WNBAFirstQuarterTotalOutcome = {
  name?: string;
  price?: number;
  point?: number;
};

type WNBAFirstQuarterTotalMarket = {
  key?: string;
  last_update?: string;
...
type WNBAFirstQuarterTotalBookmaker = {
  ...
  title?: string;
  last_update?: string;
  markets?: WNBAFirstQuarterTotalMarket[];
};
...
export async function GET(request: Request) {
  ...
  try {
      ...
      return Response.json(
        {
          available: false,
          error:
            "WNBA first-quarter totals API failed.",
          details: data,
          eventId,
          bookmakers: [],
        },
    ...
    const event =
      data as WNBAFirstQuarterTotalResponse;
    ...
    return Response.json(
      {
        ...
        message: available
          ? "WNBA first-quarter totals are available."
          : "WNBA first-quarter totals are not currently available for this matchup.",
    ...
    console.error(
      "WNBA first-quarter totals error:",
      error
    );
    ...
    return Response.json(
      {
        available: false,
        error:
          "Something went wrong fetching WNBA first-quarter totals.",
        eventId,
        bookmakers: [],
      },
      {
