export type WNBAFirstQuarterTotalOutcome = {
  name: string;
  price: number;
  point: number;
};

export type WNBAFirstQuarterTotalBookmaker = {
  key: string;
  title: string;
  lastUpdate: string | null;
  outcomes: WNBAFirstQuarterTotalOutcome[];
};

export type WNBAFirstQuarterTotalSelection = {
...
function getPriceProfile(
  price: number
): WNBAFirstQuarterTotalSelection["priceProfile"] {
  if (price < 1.55) {
...
export function findSafestWNBAFirstQuarterTotal(
  bookmakers: WNBAFirstQuarterTotalBookmaker[]
): WNBAFirstQuarterTotalSelection | null {
      ...
      (bookmaker) =>
        bookmaker.outcomes
          ...
          .map((outcome) => {
            ...
            const qualification:
              WNBAFirstQuarterTotalSelection["qualification"] =
                safetyScore >= 78 &&
                marketAlignmentScore >=
                  72 &&
                priceQualityScore >=
            ...
            const reasons = [
              `The market consensus first-quarter total is ${consensusPoint.toFixed(
  1
)}.`,
              ...
              `${supportingBookmakers} bookmakers support a comparable ${direction} first-quarter total.`,
            ];
